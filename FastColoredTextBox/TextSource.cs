...
namespace FastColoredTextBoxNS
{
    ...
    public class TextSource : IList<Line>, IDisposable
    {
        ...
        {
            if (iLine >= lines.Count) return false;
        ...
        public virtual async Task SaveToFileAsync(string fileName, Encoding enc, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                using (StreamWriter sw = new StreamWriter(fileName, false, enc))
                {
                    for (int i = 0; i < Count - 1; i++)
                    {
                        await sw.WriteLineAsync(lines[i].Text);
                        cancellationToken.ThrowIfCancellationRequested();
                    }
                    await sw.WriteAsync(lines[Count - 1].Text);
                }
            }
            finally
            {
