...
namespace FastColoredTextBoxNS
{
    ...
    public class HotkeysMapping : SortedDictionary<Keys, FCTBAction>
    {
        ...
        public static HotkeysMapping Parse(string s)
        {
            ...
            // 🔹 Alias mapping for missing key names
            Dictionary<string, KEYS> keyAliasMap = new()
            {
                { "Ins", KEYS.Insert },
                { "Del", KEYS.Delete },
                { "PgUp", KEYS.PageUp },
            ...
            foreach (var p in s.Split(','))
            {
                ...
                if (pp.Length != 2)
                {
                    throw new FormatException($"Invalid hotkey format: '{p}'. Expected format 'Key=Action'.");
                }

                var keyString = pp[0].Trim();
                var actionString = pp[1].Trim();

                // 🔹 Parse modifiers (Shift, Ctrl, Alt)
                KEYS key = KEYS.None;
                KEYS modifiers = KEYS.None;

                foreach (var part in keyString.Split('+'))
                {
                    var trimmedPart = part.Trim();

                    if (trimmedPart.Equals("Shift", StringComparison.OrdinalIgnoreCase))
                        ...
                        modifiers |= KEYS.Alt;
                    else if (keyAliasMap.TryGetValue(trimmedPart, out var mappedKey))
                        key = mappedKey;
                    else if (Enum.TryParse(trimmedPart, true, out KEYS parsedKey))
                        key = parsedKey;
                    else
                        break;
                    //throw new ArgumentException($"Unrecognized key: '{trimmedPart}'");
                }

                // 🔹 Combine modifiers and base key
                key |= modifiers;

                if (!Enum.TryParse<FCTBAction>(actionString, true, out var a))
                {
                    throw new ArgumentException($"Invalid FCTBAction value: '{actionString}'");
                }
